...
def test_name_lang_sex(card):
    ...
    expected_alt = b'\x5b' + pack('B', len(name)) + name \
               +  b'\x5f\x2d' + pack('B', len(lang)) + lang \
    ...
    assert name_lang_sex == b'' \
        or name_lang_sex == expected \
        or name_lang_sex == expected_alt \
        or name_lang_sex == expected_de \
        or name_lang_sex == expected_de_alt
...
def test_extended_capabilities(card):
    if card.is_yubikey:
        ...
        assert a == None or match(b'[\x70\x74\x75]\x00\x00\x20[\x00\x08]\x00\x00\xff\x01\x00', a) \
            or match(b'\\\x3F\x00[\x04\x08]\x00[\x04\x08]\x00[\x04\x08]\x00\x00\x01', a)
